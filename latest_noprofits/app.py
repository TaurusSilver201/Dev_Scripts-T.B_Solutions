...
def match_term(term, title):
    if title:
        ...
        for i in range(len(title_words) - len(term_words) + 1):
            if title_words[i:i+len(term_words)] == term_words:
                return True
    return False
            ...
            if response.status_code == 200:
                ...
                if results:  # Check if results are not empty
                    for nonprofits_tab_headings in results:
                        ...
                        # Check if the title or description starts with the term
                        if title.lower().startswith(term.lower()) or description.lower().startswith(term.lower()):
                            # Increment count and add revenue only for matches at the front
                            ...
                            # Break loop if the term is not at the front
                            break
