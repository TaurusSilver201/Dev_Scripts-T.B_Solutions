                    ...
                    if "hits" in jdata and jdata["hits"] > 0:
                        ...
                        if jdata['results'] == []:
                            ...
                            try:
                                ...
                                proxy = random.choice(owler_proxies)
                                _proxies = {"http": proxy, "https": proxy}
                                response = scraper.post('https://www.owler.com/iaApp/fetchCompanyProfileData.htm', json=json_data, proxies=_proxies,)
...
def extract_required_info(response_json, domain):
    ...
    rank = get_nested_value(response_json, ["layout", "data", "overview", "globalRank"])
    country = get_nested_value(response_json, ["layout", "data", "overview", "companyHeadquarterCountryCode"])
    # domain = get_nested_value(response_json, ["layout", "data", "domain"])
    op= {
        "domain": domain,
        "title": title,  
        "rank": rank,
...
def get_json_from_response(response, domain):
    ...
    script_tag = soup.find('script', text=lambda text: text and 'window.__APP_DATA__' in text)
    if script_tag:
...
def run(playwright: Playwright, domain_list) -> None:
    for search_term in domain_list:
        try:
            ...
            if op:
                with open("sample_output.csv", "a") as f:
                    f.write(str(op))
            context.close()
            browser.close()
        except:
...
def main():
    ...
    report_df.to_csv("report_" + datetime.datetime.today().strftime('%Y%d%m_%H%M%S') + ".csv", index=False, encoding='utf-8')
