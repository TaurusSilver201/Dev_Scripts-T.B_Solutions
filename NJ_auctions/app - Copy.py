...
def generating_data(searched_word):
    ...
    njurl = (f'https://www.namejet.com/Pages/Auctions/BackorderDetails.aspx?domainname={searched_word}')
    res = scraper.get(njurl).text
...
def main():
    ...
    df.loc[(df["Source"]=='0') | (df["Notes"]=="Reserve"), 'Domain'].to_csv(f"remove_{today.strftime('%Y%d%m_%H%M%S')}.txt", index=False, header=False)
